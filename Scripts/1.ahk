...
global Mewtwo, Charizard, Pikachu, Mew, Dialga, Palkia, Arceus, Shining, Solgaleo, Lunala, Buzzwole, Eevee, HoOh, Lugia, Suicune, Deluxe, MegaBlaziken, MegaGyarados, MegaAltaria, MegaCharizardY
...
MuMuv5 := isMuMuv5()
pokemonList := ["Mewtwo", "Charizard", "Pikachu", "Mew", "Dialga", "Palkia", "Arceus", "Shining", "Solgaleo", "Lunala", "Buzzwole", "Eevee", "HoOh", "Lugia", "Suicune", "Deluxe", "MegaBlaziken", "MegaGyarados", "MegaAltaria", "MegaCharizardY"]
...
showcaseLikes() {
    ...
    FindImageAndClick(174, 464, 189, 479, , "CommunityShowcase", 154, 335, 200)
...
FindOrLoseImage(X1, Y1, X2, Y2, searchVariation := "", imageName := "DEFAULT", EL := 1, safeTime := 0) {
    ...
    if(imageName = "Social" || imageName = "Country" || imageName = "Account2" || imageName = "Account" || imageName = "Points") { ;only look for deleted account on start up.
        ...
        pNeedle := GetNeedle(Path)
        ; ImageSearch within the region
        vRet := Gdip_ImageSearch_wbb(pBitmap, pNeedle, vPosXY, 30, 322, 50, 449, searchVariation)
        if (vRet = 1) {
...
FindImageAndClick(X1, Y1, X2, Y2, searchVariation := "", imageName := "DEFAULT", clickx := 0, clicky := 0, sleepTime := "", skip := false, safeTime := 0) {
    ...
    Loop { ; Main loop
        ...
        if(imageName = "Missions") { ; may input extra ESC and stuck at exit game
            ...
            pNeedle := GetNeedle(Path)
            ; ImageSearch within the region
            vRet := Gdip_ImageSearch_wbb(pBitmap, pNeedle, vPosXY, 118, 344, 135, 390, searchVariation)
            if (vRet = 1) {
                adbClick_wbb(74, 353)
                Delay(1)
            }
        ...
        if(imageName = "Skip2" || imageName = "Pack" || imageName = "Hourglass2") {
            ...
            pNeedle := GetNeedle(Path)
            vRet := Gdip_ImageSearch_wbb(pBitmap, pNeedle, vPosXY, 92, 290, 115, 317, 0)
            if(vRet = 1) {
                cantOpenMorePacks := 1
                Gdip_DisposeImage(pBitmap)
                return 0
        ...
        if(imageName = "Mission_dino2") {
            ...
            pNeedle := GetNeedle(Path)
            vRet := Gdip_ImageSearch_wbb(pBitmap, pNeedle, vPosXY, 108, 171, 177, 208, 0)
            if(vRet = 1) {
                beginnerMissionsDone := 1
                if(injectMethod && loadedAccount)
                    setMetaData()
...
SelectPack(HG := false) {
    ...
    SelectExpansionHomeTopRowY := 307
    SelectExpansionHomeSecondRowY := 441
    ...
    if(inselectexpansionscreen) {
        ...
        if(openPack == "MegaGyarados" || openPack == "MegaAltaria") {
            ...
            if (openPack == "MegaGyarados") {
                packy := SelectExpansionHomeTopRowY
...
HourglassOpening(HG := false, NEIRestart := true) {
    ...
    if(!packMethod) {
        ...
        DelayOfExtraPack -= 70
        IniWrite, %DelayOfExtraPack%, %A_ScriptDir%\..\Settings.ini, UserSettings, DelayOfExtraPack%scriptName%
        failSafe := A_TickCount
        failSafeTime := 0
        Loop {
            if(FindOrLoseImage(60, 440, 90, 480, , "HourglassPack", 1, failSafeTime)) {
